                        ...
                        <select name="dept_code" id="" class="form-control select2">
                            ...
                            @foreach($dept as $code => $name)
                                <option value="{{ $code . '|' . $name }}"
                                        {{ $info->dept_code == $code . '|' . $name ? 'selected' : '' }}>{{ '[' . $code . ']' . $name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="course_code" id="" class="form-control select2">
                            ...
                            @foreach($course as $code => $name)
                                <option value="{{ $code . '|' . $name }}"
                                {{ $info->course_code == $code . '|' . $name ? 'selected' : '' }}>{{ '[' . $code . ']' . $name }}</option>
                            @endforeach
                        </select>
                    </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            Parsley.on('form:submit', function(e) {
                ...
                return postFormData(formData, '{{ route('year-professional.update', ['id' => $info->id]) }}', 'put', true)
            });
        });
