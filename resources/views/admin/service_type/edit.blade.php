                            ...
                            <select name="parent_id" class="form-control">
                                ...
                                @if(!empty($types))
                                    @foreach($types as $type)
                                        <option value="{{ $type['id'] }}" {{ $info->parent_id == $type['id'] ? 'selected' : '' }}>{{ $type['title_display'] }}</option>
                                    @endforeach
                                @endif
                            </select>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            Parsley.on('form:submit', function (e) {
                ...
                return postFormData(formData, '{{ route('service_type.update', ['id' => $info->id]) }}', 'put', true)
            });
        });
    </script>
@endsection